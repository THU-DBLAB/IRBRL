...
class RinformanceRoute(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.route_control_sem=hub.Semaphore(1)#一次只能有一個程式掌控網路
...
# ────────────────────────────────────────────────────────────────────────────────
    ...
    def check_node_is_switch(self, ID):
        ...
        if ID[1] == None:
            ...
            return False
    def check_node_is_port(self, ID):
        # if ID=(datapath_id,None) it mean that node is switch
        if ID[1] != None:
            return True
        else:
...
# ────────────────────────────────────────────────────────────────────────────────
    ...
    def start_dynamic_tc_module(self):
        ...
        for node_id in self.G.copy().nodes:
            if self.check_node_is_port(node_id):
                ...
                for edge in list(self.G.edges()):
                    ...
                    if self.check_edge_is_link(edge_id1, edge_id2):
                        ...
                        if len(packet_end_time) != 0 or len(packet_start_time) != 0:
                            ...
                            self.G[edge_id1][edge_id2]["low_delay"] = delay_one_packet
                            self.G[edge_id1][edge_id2]["low_jitter"] = jitter
...
# ────────────────────────────────────────────────────────────────────────────────
    ...
    def _flow_removed_handler(self, ev):
        ...
        if ipv4_dst!=None:
            ...
            datapath.send_msg(mod)
            try:
                del self.G.nodes[(datapath.id,None)]["GROUP_TABLE"][msg.cookie]
            except:
                pass
            """
...
# SECTION Route Module
    ...
    def handle_route(self, datapath, port, msg):
        ...
        self.setting_multi_route_path(route_path_list_go, weight_list_go, pkt_ipv4.dst,idle_timeout=10,tos=pkt_ipv4.tos,prev_path=self.PATH[pkt_ipv4.dst][priority]["path"],prev_weight=self.PATH[pkt_ipv4.dst][priority]["weight"])
        # 上面只是規劃好路徑 這裡要幫 上來控制器詢問的`迷途小封包` 指引,防止等待rto等等...
        # 你可以測試下面刪除會導致每次pingall都要等待
        # 確保是有找到路徑
    ...
    def active_route(self):
        ...
        while True:
            self.route_control_sem.acquire()
            for dst, v in list(self.PATH.items()):
                dst_datapath_id = self.ip_get_datapathid_port[dst]["datapath_id"]
                dst_port = self.ip_get_datapathid_port[dst]["port"]
                for priority,v in list(self.PATH[dst].items()):
                    src_datapath_id_set=set()
                    if len(self.PATH[dst][priority]["path"])!=0:
                        for path in self.PATH[dst][priority]["path"]:
